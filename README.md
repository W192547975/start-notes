...
>
> **关于交互**：我是锤子手机用户，深受 **Smartisan OS** 影响。Start 的交互逻辑参考了它的经典设计——闪念胶囊、大爆炸、一步，取其精华，以现代移动端的方式重新演绎，作为对那个「工匠时代」的致敬。
...
- **拆成小步骤**：今日焦点点拆步骤图标进入——输入条写一步存一步，回车换行多写几步，**按行自动拆成多条**；写的内容还能捋一捋拆词
...
- 升级：安装过 Java 架构版（`com.start.notes`）要先卸载再安装 V1（`com.astart.app`），两个包名不同无法覆盖；数据迁移请用旧版导出 → 新版导入

## 技术与隐私

- Flutter 构建，包名 `com.astart.app`，最低 Android 8.0
