  ...
  Widget build(BuildContext context) {
    ...
    return TextPage(
      ...
      children: [
        ...
        const SizedBox(height: S.lg),
        const _H('焦点'),
        const _P('页面最上方的大标语区。没有焦点时显示「今天只做一件就好」，点「开始吧」：有事就从今日未完成里挑一件，没有就现写一件；也可以把任意任务长按拖到顶部设焦点。选好了就点「直接专注」进入专注。'),
        const _P('选定后大字显示标题，主胶囊「只做它」进入专注；另有完成、拆成小步骤、编辑、更换一件。完成后显示「主线完成，漂亮」，点「下一件」继续。'),
    ...
    return TextPage(
      ...
      children: privacy
          ...
          : const [
              ...
              _P('5. 本产品不能代替医疗。Start 是面向注意力管理挑战的辅助工具，不构成医疗建议、诊断或治疗。如有健康疑虑，请咨询专业医疗机构。'),
              _P('6. 致敬 Smartisan OS：本应用的交互理念深受锤子科技开源组件（大爆炸、一步等）启发，谨向其开源团队致敬。所有代码均为独立编写，未直接复制其源代码，相关开源组件遵循其原有许可证。'),
              _P('7. AI 辅助声明：本应用的部分界面文案、交互逻辑与代码实现由人工智能辅助生成，开发者对最终产品负责。'),
              _P('8. 继续使用即表示同意本协议；协议有更新时会再次征求你的同意。'),
            ],
    );
  }
}
            ...
            Expanded(
              child: ListView(
                ...
                children: const [
                  ...
                  _P('3. 你的所有内容仅存储在设备本地；唯一联网行为是检查软件更新（访问代码托管平台的公开接口，不发送你的任何数据）。'),
                  _P('4. 请使用设置里的导出功能自行备份；设备丢失、损坏或卸载造成的数据损失，开发者不承担责任。'),
                  _P('5. 灵感与致敬：本应用的交互灵感来源于 Smartisan OS，谨此向其团队致敬。'),
